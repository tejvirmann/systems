...
int getSize() {
    ...
    if (a < 3){  //checks correct arg size
...
MSquare *generateMSquare(int n) {
  ...
  int nsqr = n * n;  //the siamese algoritm
...
void outputMSquare(MSquare *msquare, char *filename) {
    FILE *fp;
    fp = fopen(filename, "w");  //opens file, checks if null
    ...
    for(int i = 0; i < msquare-> size; i++){ //prints the values in a file
...
int main(int argc, char *argv[]) {
    ...
    FILE *fp = fopen(*(argv + 1), "w"); //opens file 
    if (fp == NULL) {
        printf("Can't open file for reading.\n");
        exit(1);
    }
    ...
    char *name; 
    name = *(argv +1); //stores name 

    // TODO: Get magin square's size from user
    MSquare *values; //makes spaces for pointer
    values = malloc(sizeof(MSquare));
    if(values == NULL){
        exit(1);
    }
    values -> size = getSize(); //sets size 
     
    // TODO: Generate the magic square
    values = generateMSquare(values -> size); //sets size and array pointers
    
   
    // TODO: Output the magic square
    outputMSquare(values, name); //outputs to file
    ...
    free(values); //frees the pointers 
    free(clear);
    values = NULL; //sets them to null
    clear = NULL; 
